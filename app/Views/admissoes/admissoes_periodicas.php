    ...
    <div class="col-12">
        <div class="card glass-card">
            <div class="card-body">
                ...
                <div class="d-flex flex-wrap gap-2 mb-3">
                    <?php if (isset($dia_ultima_atualizacao_ofertas)): ?>
                        <span class="badge bg-light text-dark" style="font-size:0.7rem; font-weight:500;">
                            <i class="ni ni-time-alarm me-1"></i>Última oferta:
                            <?= str_pad($dia_ultima_atualizacao_ofertas, 2, "0", STR_PAD_LEFT) ?>/<?= str_pad($mes_ultima_atualizacao_ofertas, 2, "0", STR_PAD_LEFT) ?>/<?= $ano_ultima_atualizacao_ofertas ?>
                            <?= $ultima_atualizacao_ofertas ?>
                        </span>
                    <?php endif; ?>
                    <?php if (isset($dia_ultima_atualizacao_admissoes_hd)): ?>
                        <span class="badge bg-light text-dark" style="font-size:0.7rem; font-weight:500;">
                            <i class="ni ni-time-alarm me-1"></i>Última admissão HD:
                            <?= str_pad($dia_ultima_atualizacao_admissoes_hd, 2, "0", STR_PAD_LEFT) ?>/<?= str_pad($mes_ultima_atualizacao_admissoes_hd, 2, "0", STR_PAD_LEFT) ?>/<?= $ano_ultima_atualizacao_admissoes_hd ?>
                            <?= $ultima_atualizacao_admissoes_hd ?>
                        </span>
                    <?php endif; ?>
                    <?php if (isset($dia_ultima_atualizacao_admissoes_internas)): ?>
                        <span class="badge bg-light text-dark" style="font-size:0.7rem; font-weight:500;">
                            <i class="ni ni-time-alarm me-1"></i>Última admissão interna:
                            <?= str_pad($dia_ultima_atualizacao_admissoes_internas, 2, "0", STR_PAD_LEFT) ?>/<?= str_pad($mes_ultima_atualizacao_admissoes_internas, 2, "0", STR_PAD_LEFT) ?>/<?= $ano_ultima_atualizacao_admissoes_internas ?>
                            <?= $ultima_atualizacao_admissoes_internas ?>
                        </span>
                    <?php endif; ?>
                    <?php if (isset($dia_ultima_atualizacao_admissoes_externas)): ?>
                        <span class="badge bg-light text-dark" style="font-size:0.7rem; font-weight:500;">
                            <i class="ni ni-time-alarm me-1"></i>Última admissão externa:
                            <?= str_pad($dia_ultima_atualizacao_admissoes_externas, 2, "0", STR_PAD_LEFT) ?>/<?= str_pad($mes_ultima_atualizacao_admissoes_externas, 2, "0", STR_PAD_LEFT) ?>/<?= $ano_ultima_atualizacao_admissoes_externas ?>
                ...
                if (!function_exists('getCorMetaPeriodica')) {
                    function getCorMetaPeriodica($porcentagem)
                    {
                        if ($porcentagem < 75)
                    ...
                    function clampProgressPeriodica($val)
                    {
                        $rounded = (int) $val;
                ...
                <div class="row">
                    <?php foreach ($metas as $m):
                        ...
                        $pct_barra = clampProgressPeriodica($pct_geral_val);
                        $cor_barra = getCorMetaPeriodica((int) $pct_geral_val);
                        ?>
                ...
                <?php
                echo '<a href="meses?a=' . $ano_consulta . '" class="mt-4 btn bg-gradient-hmdcc text-white btn-rounded">Calendário anual</a>';
                ?>
            </div>
        </div>
    </div>
...
<script>
    function abrirModalInformacoes(dia, mes, ano) {
        ...
        if (dia == day && mes == month && year == ano) {
            ...
            $.ajax({
                url: "<?php echo site_url('/retornaDetalhesAdmissoesMesPeriodicas'); ?>",
                type: 'POST',
                dataType: "JSON",
                data: {
                    "dia": dia,
                ...
                success: function (data) {
                    ...
                    $.ajax({
                        url: "<?php echo site_url('/retornaDetalhesOfertasDiariasPeriodicas'); ?>",
                        type: 'POST',
                        dataType: "JSON",
                        data: {
                            "dia": dia,
            ...
            //ADMISSOES DIARIAS
            $.ajax({
                url: "<?php echo site_url('/retornaDetalhesAdmissoesMes'); ?>",
                type: 'POST',
                dataType: "JSON",
                data: {
                    "dia": dia,
                ...
                success: function (data) {
                    ...
                    //OFERTAS DIARIAS
                    $.ajax({
                        url: "<?php echo site_url('/retornaDetalhesOfertasDiarias'); ?>",
                        type: 'POST',
                        dataType: "JSON",
                        data: {
                            "dia": dia,
