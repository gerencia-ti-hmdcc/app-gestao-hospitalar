<div class="row">
    <?php
        ...
        for($i=1;$i<=12;$i++){
          if($i<10){
            echo("<div class='md:col-3 mb-2'><a href='/app/admissoes?m=0".$i."&a=".date("Y")."'>");
          }else{
            echo("<div class='md:col-3 mb-2'><a href='/app/admissoes?m=".$i."&a=".date("Y")."'>");
          }
          if(date('m')==$i){
            echo($calendario->draw(date(date('Y').'-'.$i.'-01')));
          }else{
            echo($calendario->draw(date(date('Y').'-'.$i.'-01'),'purple'));
          }
          echo("</a></div>");
        }
        echo("</div>");
